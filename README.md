...
- 支持商品全名或唯一简称、带或不带月份的营业额问法，以及商品营业额前 10 排名
...
1. 确定性解析器处理品类领先、商品营业额、商品排行、客单价趋势、门店比较及月份追问。商品问法可使用治理维表中的全名或唯一简称，也可以省略月份查询全部可信范围。配置 `LLM_API_KEY` 后，OpenAI 兼容模型只可把其他长尾表达映射到同一个封闭意图结构，明确禁止自行计算数字。
